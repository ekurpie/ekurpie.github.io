...
if __name__ == "__main__":
	...
	true_labels = {d['tweet_id'] : d['label'] for d in db.read_collection(collection = 'sanders_tweets_raw') }
	...
	for i, d in enumerate(D):
		...
		db.update_collection(collection = 'target_tweets_UNCW', doc = d)
	...
	labels_location = os.path.join('files', 'labels2')
	# make sure directory exists
	create_directory(labels_location)
	# save labels array
	joblib.dump(labels, os.path.join(labels_location,'labels2.pkl'))
