<?php require "includes/config.php";
    ...
    //         gender = '$info_split[0]' AND 
    //         has_assignment = 0 AND 
    //         student_id NOT IN (SELECT to_student_id 
    //                             FROM request 
    ...
    $query = "SELECT *
              FROM student
                LEFT JOIN request ON student.user_id = request.from_student_id
              WHERE user_id != $userID AND
              student.gender = '$info_split[0]' AND 
              student.has_assignment = 0 AND
...
function request_submit(form){
					...
					popup.onbeforeunload = function(){
						...
						if (document.getElementById('cancel').value == "two"){
							form.submit();
						}
            ...
            <?php
                ...
                foreach($studentArray as $s){
                    ...
                    '<div class="panel-group" id="student'.$s['id'].'">
                        ...
                        <img src='.$profilePic.' style="width:30%;"></img>
                        <table>
                            <td width="100%">
                                <h4>'.$s['name'].'</h4>
                            </td>
