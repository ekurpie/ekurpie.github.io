...
if __name__ == "__main__":
	...
	if filter_tweets:
		...
		D = db.read_collection(collection = 'raw_tweets_UNCW')

		# tracker to keep track of processed tweet IDs (in case we want to repeat the process for a set of new tweets)
		tweet_tracker = set(['{}{}'.format(x['tweet_type'], x['id']) for x in db.read_collection( collection = 'filtered_tweets_UNCW2')] )
		...
		for i, d in enumerate(D):
			...
			if not '{}{}'.format(d['tweet_type'], d['id']) in tweet_tracker:
				...
				db.insert_one_to_collection(collection = 'filtered_tweets_UNCW2', doc = d)
			else:
	...
	if clean_tweets:
		...
		# read tweets documents from database
		D = db.read_collection(collection = 'filtered_tweets_UNCW2')

		# tracker to keep track of processed tweet IDs (in case we want to repeat the process for a set of new tweets)
		tweet_tracker = set(['{}{}'.format(x['tweet_type'], x['tweet_id']) for x in db.read_collection( collection = 'target_tweets_UNCW')] )
		...
		for i, d in enumerate(D):
			...
			if not '{}{}'.format(d['tweet_type'], d['id']) in tweet_tracker:
				...
				if '{}{}'.format(d['tweet_type'], text) not in tweet_text_tracker:
					...
					db.insert_one_to_collection(collection = 'target_tweets_UNCW', doc = new_doc)
