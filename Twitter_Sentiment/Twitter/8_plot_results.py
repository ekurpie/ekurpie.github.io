...
if __name__ == "__main__":
	...
	# read labels
	labels = joblib.load(os.path.join('files', 'labels2', 'labels2.pkl'))
	...
	if create_sentiment_by_occupation:
		...
		for i, key in enumerate(['UNCW']):
			...
			for occupation, counts in values.items():
	...
	if create_twitter_tokens_bar_plot:
		...
		id_to_text = {d['tweet_id'] : d['raw_text'] for d in db.read_collection( collection = 'target_tweets_UNCW')}
