...
if __name__ == "__main__":
	...
	for mode in modes_of_research:
		...
		# tweet_tracker = set(['{}{}'.format(x['tweet_type'], x['id']) for x in db.read_collection(collection = 'raw_tweets_UNCW')])
		...
		for i, f in enumerate(F):
			...
			# check if tweet ID already processed
				...
				db.insert_one_to_collection(collection = 'raw_tweets_UNCW', doc = doc)
		...
		# 	# tracker to keep track of processed tweet ids
		# 	tweet_tracker = set(
		# 		['{}{}'.format(x['tweet_type'], x['id']) for x in db.read_collection(collection='raw_tweets_UNCW')])
		# 	# loop over each file, read content, parse relevant fields, save to db
		# 	for i, f in enumerate(F):
		# 		logging.info('Processing file {} {}/{}'.format(f, i + 1, len(F)))
		#
		...
		# 				doc['tweet_type'] = mode
		# 				# save the data of the tweet
		# 				doc['tweet_date'] = datetime.strptime(re.sub(r'[+-]([0-9])+', '', tweet['created_at']),
		# 													  '%a %b %d %H:%M:%S %Y')
		# 				# save the content of the tweet (this is the full raw content, we will parse out certain fields later on)
		# 				doc['tweet_raw'] = tweet
		#
